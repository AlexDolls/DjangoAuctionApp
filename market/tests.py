...
class ListingCreationViewTests(TestCase):
    ...
    def test_full_post_data(self):
        ...
        self.client.login(username=user_name, password=user_password)
        response = self.client.post(reverse("market:createListing"), post_data)
        self.assertURLEqual(response.url, reverse("market:details", kwargs={
            "listing_id": AuctionListing.objects.get(name="Test_Listing").id, }))
    ...
    def test_incorrect_listingname_value_post_data(self):
        ...
        self.assertEqual(response.url,
                         reverse("market:details", kwargs={"listing_id": AuctionListing.objects.get(name="1").id, }))
    ...
    def test_incorrect_imageurl_value_post_data(self):
        ...
        self.client.login(username=user_name, password=user_password)
        response = self.client.post(reverse("market:createListing"), post_data)
        self.assertEqual(response.url, reverse("market:details", kwargs={
            "listing_id": AuctionListing.objects.get(name="Test_Listing").id, }))
...
class ListingIndexViewTests(TestCase):
    def test_all_listings_are_displayed_on_page(self):
        ...
        listing_watchlist_not_active = create_listing(name="listing_1", image="None", description="test_desc",
                                                      category=category, user=user, startBid=100, days=30, active=False)
        # this listing won't be added in user's watchlist and  Not active
        listing_not_watchlist_not_active = create_listing(name="listing_2", image="None", description="test_desc",
                                                          category=category, user=user, startBid=100, days=30,
                                                          active=False)
        # this listing will be added in user's watchlist and Active
        listing_watchlist_active = create_listing(name="listing_3", image="None", description="test_desc",
                                                  category=category, user=user, startBid=100, days=30, active=True)
        # this listing won't be added in user's watchlist and Active
        listing_not_watchlist_active = create_listing(name="listing_4", image="None", description="test_desc",
                                                      category=category, user=user, startBid=100, days=30, active=True)
        ...
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_watchlist_not_active, listing_not_watchlist_not_active,
                                  listing_watchlist_active, listing_not_watchlist_active])
    ...
    def test_all_listings_are_displayed_on_page_not_logged_in(self):
        ...
        listing_not_active = create_listing(name="listing_1", image="None", description="test_desc", category=category,
                                            user=user, startBid=100, days=30, active=False)
        listing_active = create_listing(name="listing_2", image="None", description="test_desc", category=category,
...
class ListingActiveViewTests(TestCase):
    def test_only_active_listings_on_page(self):
        ...
        category = create_category(name=category_1_name)
        # this listing will be added in user's watchlist and Not active
        listing_not_active_watchlist = create_listing(name="listing_1", image="None", description="test_desc",
                                                      category=category, user=user, startBid=100, days=30, active=False)
        # this listing won't be added in user's watchlist and  Not active
        listing_not_active_not_watchlist = create_listing(name="listing_2", image="None", description="test_desc",
                                                          category=category, user=user, startBid=100, days=30,
                                                          active=False)
        # this listing will be added in user's watchlist and Active
        listing_active_watchlist = create_listing(name="listing_3", image="None", description="test_desc",
                                                  category=category, user=user, startBid=100, days=30, active=True)
        # this listing won't be added in user's watchlist and Active
        listing_active_not_watchlist = create_listing(name="listing_4", image="None", description="test_desc",
                                                      category=category, user=user, startBid=100, days=30, active=True)
        ...
        self.client.login(username=user_name, password=user_password)
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_active_watchlist, listing_active_not_watchlist])
...
class CategoryListingsViewTests(TestCase):
    ...
    def test_two_listings_different_categories_logged_in_user(self):
        ...
        listing_not_active_watchlist_category_1 = create_listing(name="listing_1", image="None",
                                                                 description="test_desc", category=category_1,
                                                                 user=user, startBid=100, days=30, active=False)
        # this listing won't be added in user's watchlist and  Not active - category_1
        listing_not_active_not_watchlist_category_1 = create_listing(name="listing_2", image="None",
                                                                     description="test_desc", category=category_1,
                                                                     user=user, startBid=100, days=30, active=False)
        # this listing will be added in user's watchlist and Active - category_1
        listing_active_watchlist_category_1 = create_listing(name="listing_3", image="None", description="test_desc",
                                                             category=category_1, user=user, startBid=100, days=30,
                                                             active=True)
        # this listing won't be added in user's watchlist and Active - category_1
        listing_active_not_watchlist_category_1 = create_listing(name="listing_4", image="None",
                                                                 description="test_desc", category=category_1,
                                                                 user=user, startBid=100, days=30, active=True)

        # this listing will be added in user's watchlist and Not active - category_2
        listing_not_active_watchlist_category_2 = create_listing(name="listing_1", image="None",
                                                                 description="test_desc", category=category_2,
                                                                 user=user, startBid=100, days=30, active=False)
        # this listing won't be added in user's watchlist and  Not active - category_2
        listing_not_active_not_watchlist_category_2 = create_listing(name="listing_2", image="None",
                                                                     description="test_desc", category=category_2,
                                                                     user=user, startBid=100, days=30, active=False)
        # this listing will be added in user's watchlist and Active - category_2
        listing_active_watchlist_category_2 = create_listing(name="listing_3", image="None", description="test_desc",
                                                             category=category_2, user=user, startBid=100, days=30,
                                                             active=True)
        # this listing won't be added in user's watchlist and Active - category_2
        listing_active_not_watchlist_category_2 = create_listing(name="listing_4", image="None",
                                                                 description="test_desc", category=category_2,
                                                                 user=user, startBid=100, days=30, active=True)

        user.watchlist.add(listing_active_watchlist_category_1, listing_not_active_watchlist_category_1,
                           listing_active_watchlist_category_2, listing_not_active_watchlist_category_2)

        self.client.login(username=user_name, password=user_password)
        response = self.client.get(reverse("market:category_listings", kwargs={"category_id": category_1.id}))
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_not_active_watchlist_category_1, listing_not_active_not_watchlist_category_1,
                                  listing_active_watchlist_category_1, listing_active_not_watchlist_category_1])
...
class MyListingsViewTests(TestCase):
    def test_no_my_listings(self):
        ...
        # Not active - user_2
        listing_not_active_user_2 = create_listing(name="listing_1", image="None", description="test_desc",
                                                   category=category_1, user=user_2, startBid=100, days=30,
                                                   active=False)
        # Active - user_2
        listing_active_user_2 = create_listing(name="listing_4", image="None", description="test_desc",
                                               category=category_1, user=user_2, startBid=100, days=30, active=True)
    ...
    def test_exist_only_my_listings(self):
        ...
        # Not active - user_1
        listing_not_active_user_1 = create_listing(name="listing_1", image="None", description="test_desc",
                                                   category=category_1, user=user_1, startBid=100, days=30,
                                                   active=False)
        # Active - user_1
        listing_active_user_1 = create_listing(name="listing_4", image="None", description="test_desc",
                                               category=category_1, user=user_1, startBid=100, days=30, active=True)

        self.client.login(username=user_name_1, password=user_password_1)
        response = self.client.get(reverse("market:mylistings"))
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_not_active_user_1, listing_active_user_1])
    ...
    def test_exist_not_only_my_listings_watchlist(self):
        ...
        user_1.watchlist.add(listing_not_active_user_2, listing_active_user_2, listing_not_active_user_1,
                             listing_active_user_1)
        self.client.login(username=user_name_1, password=user_password_1)
        response = self.client.get(reverse("market:mylistings"))
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_not_active_user_1, listing_active_user_1])
...
class WatchlistViewTests(TestCase):
    ...
    def test_unauthorized_user_empty_query_POST_request_add_to_watchlist(self):
        ...
        user_1 = create_user(username=user_name_1, password=user_password_1)
        category_1 = create_category(name=category_1_name)
        listing = create_listing(name="listing_1", image="None", description="test_desc", category=category_1,
                                 user=user_1, startBid=100, days=30, active=True)
    ...
    def test_own_and_other_owner_listings_in_watchlist(self):
        ...
        response = self.client.get(reverse("market:watchlist"))
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_active_user_1, listing_not_active_user_1, listing_active_user_2,
                                  listing_not_active_user_2], ordered=False)
    ...
    def test_only_other_user_listings_in_watchlist(self):
        ...
        response = self.client.get(reverse("market:watchlist"))
        self.assertQuerysetEqual(list(response.context['active_listing_list']),
                                 [listing_not_active_user_2, listing_active_user_2])
...
class DetailsViewTests(TestCase):
    def test_active_listing_detail_page_unauthorized(self):
        ...
        user_1 = create_user(username=user_name_1, password=user_password_1)
        category_1 = create_category(name=category_1_name)
        listing_active_user_1 = create_listing(name="listing_1", image="None", description="test_desc",
                                               category=category_1, user=user_1, startBid=100, days=30, active=True)
    ...
    def test_not_active_listing_detail_page_unauthorized(self):
        ...
        category_1 = create_category(name=category_1_name)
        listing_active_user_1 = create_listing(name="listing_1", image="None", description="test_desc",
                                               category=category_1, user=user_1, startBid=100, days=30, active=False)
        response = self.client.get(reverse("market:details", kwargs={"listing_id": listing_active_user_1.id}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["true_user"], False)
        self.assertEqual(response.context['auctionlisting'], listing_active_user_1)
